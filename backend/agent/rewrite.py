...
1. **绝对不能新增用户没提到的领域**。例如：
   ...
   - 用户说"唱歌" → 只能涉及 play(KTV) 领域，不能加 eat/drink
...
## 输出 JSON 格式
{
  ...
  "missing_info": ["缺失的信息"],
  "constraints": {
    "time_window_hint": "lunch|dinner|afternoon|evening|null",
...
def _detect_domains_from_message(message: str) -> list[str]:
    ...
    eat_kw = ["吃", "饭", "餐厅", "美食", "火锅", "烧烤", "日料", "西餐", "中餐", "小吃",
              ...
              "午餐", "晚饭", "晚餐", "宵夜", "夜宵", "甜点"]
    play_kw = ["玩", "唱歌", "KTV", "电影", "展览", "博物馆", "游乐园", "公园", "运动",
               "健身", "游泳", "攀岩", "密室", "剧本杀", "桌游", "麻将", "逛街", "购物",
               "拍照", "打卡", "演出", "音乐会", "话剧", "脱口秀", "相声"]
...
def _detect_time_window(message: str) -> str | None:
    """检测时间窗口"""
    if any(kw in message for kw in ["早餐", "上午", "早上", "早饭", "早点"]):
        return "morning"
    if any(kw in message for kw in ["午餐", "中午", "午饭", "午餐"]):
        return "lunch"
    if any(kw in message for kw in ["下午", "午后"]):
        return "afternoon"
    if any(kw in message for kw in ["晚餐", "晚饭", "傍晚", "晚上"]):
        return "dinner"
    if any(kw in message for kw in ["宵夜", "夜宵", "深夜", "半夜"]):
        return "night"
    return None
